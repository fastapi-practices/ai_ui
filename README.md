...
## 添加 @antdv-next/x-markdown 和 @antdv-next/x-sdk 依赖
...
这一步表示 `@vben/web-antdv-next` 应用依赖 `@antdv-next/x-markdown` 和 `@antdv-next/x-sdk`，具体来源由根目录 `catalog` 统一控制
...
```bash
pnpm --filter @vben/web-antdv-next add @antdv-next/x-markdown@catalog: @antdv-next/x-sdk@catalog:
```

执行后请确认：

- `pnpm-workspace.yaml` 中存在 `@antdv-next/x-markdown` 和 `@antdv-next/x-sdk` 的 catalog 条目
