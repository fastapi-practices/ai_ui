...
type ChatMessageItem = AIChatMessageDetail & {
  id: string;
  streaming?: boolean;
};
...
const COMPOSER_DEFAULT_HEIGHT = 56;
const COMPOSER_FALLBACK_MAX_HEIGHT = 420;
const MIN_MESSAGE_VIEWPORT_HEIGHT = 180;
const STOP_SEQUENCES_PLACEHOLDER = '["</thinking>"]';
const EXTRA_HEADERS_PLACEHOLDER = '{"x-trace-id":"chat-demo"}';
const EXTRA_BODY_PLACEHOLDER = '{"reasoning":{"effort":"medium"}}';
const LOGIT_BIAS_PLACEHOLDER = '{"198":-100}';
...
function buildMessageId(seedValue?: null | number | string) {
...
function parseDateLabel(value?: null | string, withDate = true) {
  if (!value) {
    return '-';
  }
...
function normalizeMessage(
  ...
  return {
    ...
    conversation_id: item.conversation_id ?? activeConversationId.value ?? null,
...
function applyStreamMessage(data: AIChatMessage) {
  ...
  if (data.role === 'user') {
    activeMessages.value.push(
      normalizeMessage(data, activeMessages.value.length),
    );
  } else {
...
async function submitChat(
  ...
  const promptText =
    regenerateMessageIndex === undefined ? prompt.value.trim() : undefined;
  ...
  try {
    payload = {
      ...
      extra_body: parseJsonField<Record<string, any> | string>(
        extraBody.value,
        '额外请求体',
      ),
            ...
            <div class="ai-composer__shell">
              ...
              <div class="ai-composer__footer">
                    ...
                    <template #content>
                      <div class="w-[320px]">
                        ...
                        <div
                          ...
                          class="flex max-h-[220px] flex-wrap gap-2 overflow-y-auto"
                        >
                          <button
                            v-for="item in quickPhrases"
                            :key="item.id"
                            class="rounded-full border border-border bg-background px-3 py-1 text-xs text-muted-foreground transition-colors hover:border-primary/30 hover:text-foreground"
                            :title="item.content"
                            type="button"
                ...
                <div class="ai-composer__meta">
                  ...
                  <button
                    v-if="editingMessage"
                    class="ai-composer__meta-action"
                    type="button"
...
.ai-composer__meta-action:hover {
  color: hsl(var(--foreground));
}
